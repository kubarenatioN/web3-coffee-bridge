...
import {CoffeeTokenL2} from "../src/L2/CoffeeTokenL2.sol";

contract CoffeeTokenL2Deployer is Script {
    string deployConfig;
    ...
    {
        address bridge = vm.parseTomlAddress(deployConfig, ".token.bridge");
        address remoteToken = vm.parseTomlAddress(
            deployConfig,
            ".token.remote_token"
        );
        ...
        if (preComputedAddress.code.length > 0) {
            ...
            addr_ = address(
                new CoffeeTokenL2{salt: _implSalt()}(
                    bridge,
                    remoteToken,
                    ownerAddr_,
                    name,
            ...
            console.log(
                "Deployed CoffeeToken at address: ",
                addr_,
                "on chain id: ",
                block.chainid
            );
